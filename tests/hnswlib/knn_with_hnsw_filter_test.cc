...
namespace {
    ...
    void test_some_hnsw_filtering(phekda::SearchCondition &filter_func, phekda::BaseFilterFunctor&filter_base, size_t div_num, size_t label_id_start) {
        int d = 4;
        idx_t n = 100;
        idx_t nq = 10;
        size_t k = 10;
        ...
        flat_config.index_type = phekda::IndexType::INDEX_HNSWLIB;
        ...
        auto *alg_hnsw = phekda::UnifiedIndex::create_index(flat_config.index_type);
        auto rs = alg_hnsw->initialize(flat_index_config);
        if (!rs.ok()) {
        ...
        config.space = &space;
        phekda::AlgorithmInterface *alg_hnsw_base = new phekda::HierarchicalNSW();
        rs = alg_hnsw_base->initialize(flat_config, config);
        if (!rs.ok()) {
            std::cout << "Failed to initialize FLAT HNSW: " << rs << std::endl;
            exit(1);
        }
        for (size_t i = 0; i < n; ++i) {
            ...
            rs = alg_hnsw->add_vector(reinterpret_cast<const uint8_t *>(data.data() + d * i), label_id_start + i);
            if (!rs.ok()) {
                std::cout << "Failed to add point to FLAT HNSW: " << rs << std::endl;
                exit(1);
            }
            rs = alg_hnsw_base->addPoint(data.data() + d * i, label_id_start + i, {false});
            if (!rs.ok()) {
                std::cout << "Failed to add point to FLAT HNSW: " << rs << std::endl;
                exit(1);
            }
        ...
        for (size_t j = 0; j < nq; ++j) {
            ...
            auto gd = alg_hnsw->search(context);
            auto res = alg_hnsw_base->searchKnnCloserFirst(p, k, &filter_base);
            std::cout<<"Results:\n";
...
TEST(Hnswlib, hnsw){
    ...
    test_some_hnsw_filtering(pickIdsDivisibleByThree, pickIdsDivisibleByThreeBase, 3, 17);
    ...
    test_some_hnsw_filtering(pickIdsDivisibleBySeven, pickIdsDivisibleBySevenBase, 7, 17);
    ...
    test_some_hnsw_filtering(pickIdsDivisibleByThirteenCondition, pickIdsDivisibleByThirteen, 13, 21);
