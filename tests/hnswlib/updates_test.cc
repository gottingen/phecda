...
static float
test_approx(std::vector<float> &queries, size_t qsize, phekda::HierarchicalNSW &appr_alg, size_t vecdim,
            std::vector<std::unordered_set<phekda::LabelType>> &answers, size_t K) {
    size_t correct = 0;
    size_t total = 0;

    for (int i = 0; i < qsize; i++) {
        std::priority_queue<std::pair<phekda::DistanceType, phekda::LabelType>> result = appr_alg.searchKnn((char *)(queries.data() + vecdim * i), K);
        total += K;
        while (result.size()) {
...
test_vs_recall(
    ...
    size_t qsize,
    phekda::HierarchicalNSW &appr_alg,
    size_t vecdim,
    std::vector<std::unordered_set<phekda::LabelType>> &answers,
    size_t k) {
...
int main(int argc, char **argv) {
    ...
    phekda::L2Space l2space(d);
    phekda::HierarchicalNSW appr_alg(&l2space, N + 1, M, efConstruction);
    ...
    std::vector<std::unordered_set<phekda::LabelType>> answers(N_queries);
