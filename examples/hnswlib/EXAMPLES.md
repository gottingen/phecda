...
int main() {
    ...
    float correct = 0;
    for (int i = 0; i < max_elements; i++) {
        std::priority_queue<std::pair<float, phekda::LabelType>> result = alg_hnsw->searchKnn(data + i * dim, 1);
    ...
    alg_hnsw = new phekda::HierarchicalNSW<float>(&space, hnsw_path);
    correct = 0;
    for (int i = 0; i < max_elements; i++) {
        std::priority_queue<std::pair<float, phekda::LabelType>> result = alg_hnsw->searchKnn(data + i * dim, 1);
        phekda::LabelType label = result.top().second;
...
int main() {
    ...
    int k = 10;
    for (int i = 0; i < max_elements; i++) {
        std::vector<std::pair<float, phekda::LabelType>> result = alg_hnsw->searchKnnCloserFirst(data + i * dim, k, &pickIdsDivisibleByTwo);
...
int main() {
    ...
    // Initing index
    phekda::L2Space space(dim);
    phekda::HierarchicalNSW<float>* alg_hnsw = new phekda::HierarchicalNSW<float>(&space, max_elements, M, ef_construction, 100, true);
